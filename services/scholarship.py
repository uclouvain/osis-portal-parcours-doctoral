...
class DoctorateScholarshipAPIClient:
    def __new__(cls):
        api_config = admission_sdk.build_configuration()
        return scholarship_api.ScholarshipApi(ApiClient(configuration=api_config))


class DoctorateScholarshipService(metaclass=ServiceMeta):
    api_exception_cls = ApiException

    @classmethod
    def get_scholarship(cls, person, scholarship_uuid):
        return DoctorateScholarshipAPIClient().retrieve_scholarship(
